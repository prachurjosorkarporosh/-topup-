    ...
    function loadGlobalSettings() {
        db.collection("settings").doc("general").onSnapshot((doc) => {
            if (doc.exists) {
                ...
                setSafeSrc('header-logo-img', logoUrl);
                setSafeSrc('about-logo-img', logoUrl);
                setSafeText('about-site-name', siteName);

                // Sync logos for new payment UI
                setSafeSrc('step2-logo', logoUrl);
                setSafeSrc('step2-center-logo', logoUrl);
                setSafeSrc('pay-site-logo-small', logoUrl);
    ...
    auth.onAuthStateChanged((user) => {
        if (user) {
            ...
            const authSecEl = document.getElementById('auth-sec');
            if(authSecEl && !authSecEl.classList.contains('hidden')){
                showSec('home');
            }
            ...
            // Reset profile data
            const emptyName = "-";
            setSafeText('acc-profile-name', emptyName);
            setSafeText('acc-email', emptyName);

            // Guard protected pages
            const path = window.location.pathname;
            if (path.includes('history') || path.includes('account') || path.includes('addmoney')) {
                window.location.href = '/auth.html?returnUrl=' + encodeURIComponent(path + window.location.search);
            }
        }
    });
    ...
    function loadUserData(uid) {
        ...
        userDocUnsub = db.collection('users').doc(uid).onSnapshot((doc) => {
            if (doc.exists) {
                ...
                // Update Header
                setSafeText('header-balance', userBalance);
                setSafeText('topup-balance-display', userBalance.toFixed(2));

                // Update New Account Section
                setSafeText('acc-profile-name', userName);
                setSafeText('acc-email', userEmail);
                setSafeText('acc-profile-balance', userBalance);
                setSafeText('acc-balance-card', userBalance.toFixed(2));
                setSafeText('acc-support-pin', uid.substring(0, 6).toUpperCase());
                ...
                if(imgContainer && initialContainer) {
                    if(data.photoURL) {
    ...
    function loadAppContent() {
        db.collection("settings").doc("general").onSnapshot((doc) => {
            if(doc.exists) {
                const data = doc.data();
                if(data.notice) setSafeText('notice-text', data.notice);
                ...
                setSafeHref('home-whatsapp', waLink);
                setSafeHref('home-telegram', tgLink);
                ...
                setSafeHref('link-whatsapp', waLink);
                setSafeHref('link-telegram', tgLink);
                setSafeHref('about-facebook', fbLink);
                setSafeHref('about-youtube', ytLink);
                setSafeHref('about-telegram', tgLink);
                setSafeHref('about-whatsapp', waLink);
                setSafeHref('contact-whatsapp', waLink);
                setSafeHref('contact-telegram', tgLink);
                setSafeHref('contact-facebook', fbLink);
            }
        });

        if (document.getElementById('services-list')) loadGamesAndCategories();
        if (document.getElementById('tutorial-list-container')) loadTutorials();
        if (document.getElementById('banner-slider') || document.getElementById('slider-wrapper')) loadBanners();
        if (document.getElementById('latest-orders-list')) loadRecentOrdersGlobal();
        if (document.getElementById('topup-sec')) initTopUpPage(); 
    }
    ...
    function initTopUpPage() {
        const topupSec = document.getElementById('topup-sec');
        if (!topupSec) return;
        ...
        if (gameId) {
            db.collection('services').doc(gameId).get().then(doc => {
                if (doc.exists) {
                    gamesData[gameId] = doc.data();
                    openTopUpPage(gameId, doc.data().name || gameName || 'Game', doc.data().image || gameImg || '');
                } else {
                    openTopUpPage(gameId, gameName || 'Game', gameImg || '');
                }
            }).catch(() => {
                openTopUpPage(gameId, gameName || 'Game', gameImg || '');
            });
        } else {
            db.collection('services').limit(1).get().then(snap => {
                if (!snap.empty) {
                    const sDoc = snap.docs[0];
                    gamesData[sDoc.id] = sDoc.data();
                    openTopUpPage(sDoc.id, sDoc.data().name, sDoc.data().image);
                }
            });
        }
    }
    ...
    function openTopUpPage(gameId, gameName, gameImg) {
        const topupSec = document.getElementById('topup-sec');
        if (!topupSec) {
            window.location.href = '/topup.html?gameId=' + encodeURIComponent(gameId) + '&name=' + encodeURIComponent(gameName || '') + '&img=' + encodeURIComponent(gameImg || '');
            return;
        }
        ...
        if (!gamesData[gameId]) {
            gamesData[gameId] = { name: gameName, image: gameImg, type: 'uid' };
        }
        selectedGameType = gamesData[gameId].type || 'uid'; 
        
        selectPaymentOption('wallet');
        isDirectBuy = false;

        const totalPriceEl = document.getElementById('total-price');
        if (totalPriceEl) totalPriceEl.innerText = "0";
        const gamePlayerIdEl = document.getElementById('game-player-id');
        if (gamePlayerIdEl) gamePlayerIdEl.value = "";
        setSafeText('topup-game-title', gameName);
        setSafeText('topup-sub-title', gameName + " ");
        setSafeSrc('topup-game-img', gameImg);

        const playerSection = document.getElementById('player-id-section');
        if (playerSection) playerSection.classList.toggle('hidden', selectedGameType === 'code');
        
        const topupSubTitle = document.getElementById('topup-sub-title');
        if (topupSubTitle) {
            if (selectedGameType === 'code') {
                topupSubTitle.innerText = gameName + " (Instant Voucher)";
            } else if (selectedGameType === 'auto-topup') {
                topupSubTitle.innerText = gameName + " (Auto Delivery)";
            } else {
                topupSubTitle.innerText = gameName + " (Player ID)";
            }
        }
        ...
        if(gamesData[gameId] && gamesData[gameId].rules) {
            ...
            if (rulesText) rulesText.innerHTML = rulesHtml || gamesData[gameId].rules;
    ...
    function showSec(secId) {
        ...
        if ((secId === 'history' || secId === 'account' || secId === 'addmoney') && !currentUser) {
            window.location.href = '/auth.html?returnUrl=' + encodeURIComponent(pageRoutes[secId] || '/');
            return;
        }

        const secEl = document.getElementById(secId + '-sec');
        if (secEl) {
            ...
            if(noticeSection) {
                if(secId === 'home') noticeSection.classList.remove('hidden');
                else noticeSection.classList.add('hidden');
            }
            
            if(secId === 'addmoney') {
                isDirectBuy = false;
                const am1 = document.getElementById('am-step-1');
                const am2 = document.getElementById('am-step-2');
                const am3 = document.getElementById('am-step-3');
                if(am1) am1.classList.remove('hidden');
                if(am2) am2.classList.add('hidden');
                if(am3) am3.classList.add('hidden');
            }
            if(secId === 'history') {
                loadHistory();
            }
        ...
        } else if (pageRoutes[secId]) {
            window.location.href = pageRoutes[secId];
        }
    }
